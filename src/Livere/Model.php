 ...
 *
 */
class Livere_Model implements ArrayAccess
{
    /**
     * If you need to specify a NULL JSON value, use Livere_Model::NULL_VALUE
    ...
    protected function mapTypes($array)
    {
        ...
        foreach ($array as $key => $val) {
            if (!property_exists($this, $this->keyType($key)) && property_exists($this, $key)) {
                $this->$key = $val;
                unset($array[$key]);
            } elseif (property_exists($this, $camelKey = Livere_Utils::camelCase($key))) {
    ...
    {
        if ($value instanceof Livere_Model) {
     ...
     * @throws Livere_Exception Thrown if $obj isn't an array.
    ...
    {
        if ($obj && !is_array($obj)) {
            throw new Livere_Exception("Incorrect parameter type passed to $method(). Expected an array.");
        }
    }
