 ...
 */
class Livere_Cache_File extends Livere_Cache_Abstract
{
    ...
    /**
     * @var Livere_Client the current client
     */
    private $client;
    ...
    public function get($key, $expiration = false)
    {
        ...
        if ($this->acquireReadLock($storageFile)) {
            if (filesize($storageFile) > 0) {
                ...
                $this->client->getLogger()->debug('Cache file was empty', array('file' => $storageFile));
            }
    ...
    public function delete($key)
    {
        ...
        if (file_exists($file) && !unlink($file)) {
            ...
            throw new Livere_Cache_Exception("Cache file could not be deleted");
        }
    ...
    private function getCacheDir($file, $forWrite)
    {
        ...
        if ($forWrite && !is_dir($storageDir)) {
            if (!mkdir($storageDir, 0700, true)) {
                $this->client->getLogger()->error('File cache creation failed', array('dir' => $storageDir));
                throw new Livere_Cache_Exception("Could not create storage directory: $storageDir");
            }
        }
        return $storageDir;
    }
    ...
    private function acquireWriteLock($storageFile)
    {
        ...
        if (!$rc) {
            $this->client->getLogger()->notice('File cache write lock failed', array('file' => $storageFile));
    ...
    private function acquireLock($type, $storageFile)
    {
        ...
        if (!$this->fh) {
            $this->client->getLogger()->error('Failed to open file during lock acquisition', array('file' => $storageFile));
            return false;
        }
        if ($type == LOCK_EX) {
