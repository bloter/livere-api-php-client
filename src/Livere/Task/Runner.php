...
class Livere_Task_Runner
{
    ...
    /**
     * @var Livere_Client $client The current API client.
     */
    private $client;
     ...
     *
     * @param Livere_Client $client The current API client
     * @param string $name The name of the current task (used for logging)
     * @param callable $action The task to run and possibly retry
     * @param array $arguments The task arguments
     * @throws Livere_Task_Exception when misconfigured
     */
    public function __construct(Livere_Client $client, $name, $action, array $arguments = array())
    {
        $config = (array)$client->getClassConfig('Livere_Task_Runner');

        if (isset($config['initial_delay'])) {
            if ($config['initial_delay'] < 0) {
                throw new Livere_Task_Exception('Task configuration `initial_delay` must not be negative.');
            }
        ...
        if (isset($config['max_delay'])) {
            if ($config['max_delay'] <= 0) {
                throw new Livere_Task_Exception('Task configuration `max_delay` must be greater than 0.');
            }
        ...
        if (isset($config['factor'])) {
            if ($config['factor'] <= 0) {
                throw new Livere_Task_Exception('Task configuration `factor` must be greater than 0.');
            }
        ...
        if (isset($config['jitter'])) {
            if ($config['jitter'] <= 0) {
                throw new Livere_Task_Exception('Task configuration `jitter` must be greater than 0.');
            }
        ...
        if (isset($config['retries'])) {
            if ($config['retries'] < 0) {
                throw new Livere_Task_Exception('Task configuration `retries` must not be negative.');
            }
            $this->maxAttempts += $config['retries'];
        }

        if (!is_callable($action)) {
            throw new Livere_Task_Exception('Task argument `$action` must be a valid callable.');
        }
     ...
     *
     * @return mixed
     * @throws Livere_Task_Retryable on failure when no retries are available.
     */
    public function run()
    {
        while ($this->attempt()) {
            ...
            } catch (Livere_Task_Retryable $exception) {
    ...
    private function backOff()
    {
        ...
        $this->client->getLogger()->debug('Retrying task with backoff', array(
            'request' => $this->name,
            'retry' => $this->attempts,
