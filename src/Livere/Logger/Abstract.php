 ...
 */
abstract class Livere_Logger_Abstract
{
    /**
     * Default log format
     */
    ...
    /**
     * @param Livere_Client $client  The current Livere client
     */
    public function __construct(Livere_Client $client)
    {
        $this->setLevel($client->getClassConfig('Livere_Logger_Abstract', 'level'));

        $format = $client->getClassConfig('Livere_Logger_Abstract', 'log_format');
        $this->logFormat = $format ? $format : self::DEFAULT_LOG_FORMAT;

        $format = $client->getClassConfig('Livere_Logger_Abstract', 'date_format');
        $this->dateFormat = $format ? $format : self::DEFAULT_DATE_FORMAT;

        $this->allowNewLines = (bool)$client->getClassConfig('Livere_Logger_Abstract', 'allow_newlines');
    }
     ...
     * @throws Livere_Logger_Exception If $level is invalid
     */
    protected function normalizeLevel($level)
    {
        ...
        throw new Livere_Logger_Exception(sprintf("Unknown LogLevel: '%s'", $level));
    }
